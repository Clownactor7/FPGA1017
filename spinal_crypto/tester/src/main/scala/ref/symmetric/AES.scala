...
object AES {
  ...
  def block(
      keyLenght: Int,
      verbose: Boolean
  )(key: BigInt, block: BigInt, enc: Boolean) = if (enc)
    encryptBlock(key, block, keyLenght, verbose)
  else decryptBlock(key, block, keyLenght, verbose)
  ...
  def encryptBlock(
    ...
    if (verbose) {
      println(
        s"Plain  : 0x${blockPlain.map(b => "%02X".format(b)).mkString("")}"
      )
      println(
        s"KEY    : 0x${myKey.getEncoded().map(b => "%02X".format(b)).mkString("")}"
      )
      println(
        s"Cipher : 0x${blockCipher.map(b => "%02X".format(b)).mkString("")}"
      )
      println("")
    }
...
//  println(bigIntToHex(plain))
  def getSecretKeySpec(
    ...
    val secretKey: SecretKey = SecretKeyFactory
      .getInstance(
        "PBEWithMD5AndDES"
      )
  ...
  def encrypt(
    ...
    // System.out.println("skeySpec : " + asHexStr(skeySpec))
    val cipher: Cipher = Cipher.getInstance("AES/ECB/NoPadding")
    // System.out.println(asHexStr(skeySpec.getAlgorithm))
  ...
  var encrypted: Array[Byte] = encrypt(
    message.getBytes,
    "1111111122222222333333334444444411111111222222223333333344444444",
    "AES",
    256
  )
